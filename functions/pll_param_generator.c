...
void Set_PLL (void *addr, uint32_t counter_select, double out_freq, double duty_cycle, uint32_t enable_message) {
	...
	if (pll_calculator (pll_param, out_freq, INPUT_FREQ)) { // frequency can be implemented
		Set_M(addr, pll_param, DISABLE_MESSAGE);
		Set_MFrac (addr, pll_param, DISABLE_MESSAGE);
		Set_N(addr, pll_param, DISABLE_MESSAGE);
		Set_C (addr, pll_param, counter_select, duty_cycle, DISABLE_MESSAGE);
		...
		if (enable_message) {
			...
			printf("PLL Counter [%02d]\n",counter_select);
			printf("\tFrequency \t\t: %5.2f MHz\n",temp);
